...
mydb = mysql.connector.connect(host="dbeia.chfege0xcxax.us-east-1.rds.amazonaws.com",
                               user="admin", password="admineia", database="dbeia")
...
async def create_current_dot(dot: Dot_current):
    try:
        mydb = mysql.connector.connect(host="dbeia.chfege0xcxax.us-east-1.rds.amazonaws.com",
                                       user="admin", password="admineia", database="dbeia")
        mycursor = mydb.cursor(buffered=True, dictionary=True)
        mycursor.execute("INSERT INTO dbeia.eia_api_dots (value,units,devices_id) VALUES (%s,%s,%s)",
                         (dot.value, dot.units, dot.devices_id))
        mydb.commit()
        return {"dot": dot}
    except Exception as e:
        print("error", e)
...
async def create_device(devices: Device):
    try:
        mydb = mysql.connector.connect(host="dbeia.chfege0xcxax.us-east-1.rds.amazonaws.com",
                                       user="admin", password="admineia", database="dbeia")
        mycursor = mydb.cursor(buffered=True, dictionary=True)
        mycursor.execute("INSERT INTO dbeia.eia_api_devices (device_name,secret,machines_id) VALUES (%s,%s,%s)",
                         (devices.name, devices.secret, devices.machines_id))
        mydb.commit()
...
async def websocket_endpoint(websocket: WebSocket):
    try:
        ...
        while True:
            try:
                # cliennte envia mensaje
                text = await websocket.receive_text()
                print("mensaje enviado:", text)
                # resp={"valor respuesta:": random.uniform(0,1)}
                resp = {"sender": "Servidor",
                        "message": round(random.uniform(0, 1), 2)}
